...
@router.get("/export")
def export_events(session: Session = Depends(get_session)):
    """Exports all events to CSV."""
    statement = select(KKEvent).order_by(KKEvent.timestamp.desc())
    ...
    # Header
    writer.writerow(["ID", "UsuarioID", "Consistencia", "Notas", "Fecha (ISO)", "Fecha (Local)"])
    
    # Rows
    for event in events:
        # Format local date roughly for Excel readability
        local_date = event.timestamp.strftime("%Y-%m-%d %H:%M:%S")
        writer.writerow([
            event.id, 
            event.user_id, 
            event.consistency.value if event.consistency else "N/A", 
            event.notes, 
    ...
    output.seek(0)
    return Response(content=output.getvalue(), media_type="text/csv", headers={"Content-Disposition": "attachment; filename=kk_logs.csv"})
