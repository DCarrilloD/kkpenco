...
### [x] 1.4 Notificaciones push muertas 🔴 — HECHO (2026-07-06; despliegue de Functions pendiente de ejecutar por el usuario)
...
- **Fix aplicado (Parte B — servidor)**: `flutter_app/functions/` (JS, `firebase-functions` v2 + `firebase-admin`) con triggers `onEventCreated`, `onDuelCreated`, `onDuelUpdated` (aceptado/finalizado) y `onChatCreated` (opt-in); gating por `notifPrefs` con los mismos defaults que el cliente; limpieza de tokens caducados. Registrado en `firebase.json`. **Pendiente**: `cd flutter_app/functions && npm install && cd .. && firebase deploy --only functions` (Blaze activo) + alerta de presupuesto en Google Cloud. Sintaxis validada con `node --check`; sin cambios de reglas.
