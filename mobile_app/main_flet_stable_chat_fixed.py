...
async def main(page: ft.Page):
    ...
    def view_login():
        ...
        def handle_recovery(e):
             ...
             if success:
                 page.snack_bar = ft.SnackBar(ft.Text("Contraseña temporal enviada a tu email (Revisa la consola)"))
                 page.snack_bar.open = True
                 # Go back to login to let them enter the new password
                 toggle_mode_login(None)
             else:
                 page.snack_bar = ft.SnackBar(ft.Text("Error: No se pudo enviar el correo."))
                 page.snack_bar.open = True
                 page.update()
                ...
                ft.Container(
                    ...
                    content=ft.Column([
                        ...
                        ft.Container(
                            ...
                            bgcolor="transparent", # Seamless integration
                            border_radius=15,
                            alignment=ft.Alignment(0, 0),
                            content=ft.Image(
                                src="assets/chivi.png",
                                fit="contain",
                                width=None,
                                height=None,
                            ),
                        ...
                        card = ft.Container(
                            ...
                            content=ft.Column([
                                ft.Row([
                                    # Delete Button
                                    ft.Container(
                                        content=ft.Icon("close", color="red"),
                                        width=40, height=40,
                                        alignment=ft.Alignment(0, 0),
                                        border_radius=20,
                                        data=event_id, 
    ...
    def view_cos():
         ...
         def load_messages():
             if not api:
                 messages_list.controls = [ft.Text("Chat no disponible offline.", color="red", text_align="center")]
                 page.update()
                 return False

             msgs = api.get_chat_messages()
             ...
             for m in msgs:
                 ...
                 # Bubble Style
                 bubble_color = ACCENT_COLOR if is_me else "#424242"
                 ...
                 msg_content = ft.Column(
                     spacing=2,
                     controls=[
                         ft.Text(username, size=12, color="grey", weight="bold") if not is_me else ft.Container(),
                         ft.Container(
                             ...
                             content=ft.Column([
                                 ft.Text(content, color=text_color, size=15),
                                 ft.Text(time_display, color=text_color if is_me else "grey", size=10, text_align="right")
                             ], spacing=2)
                         )
                     ]
                 )
                 ...
                 messages_list.controls.append(
                     ft.Row([msg_content], alignment=alignment)
                 )
             
             # Only update if page is still active
             try:
                 page.update()
             except:
                 pass
         ...
         def send_message(e):
             if not api or not input_field.value: return
             
             if api.send_chat_message(current_user["id"], input_field.value):
                 input_field.value = ""
                 load_messages() # Refresh immediately
                 input_field.focus()
             else:
                 page.snack_bar = ft.SnackBar(ft.Text("Error al enviar mensaje"))
                 page.snack_bar.open = True
                 page.update()
         ...
         # Note: Polling should be handled globally or via a loop, 
         # but for simplicity we rely on manual refresh or we can add a simple async loop.
         # For this iteration, we load on entry. 
         # Real-time polling requires a safer threaded approach to avoid blocking UI.
         # We will add a simple recursive timer for this view.
         
         def poll_loop():
            # Check if we are still on the chat tab (index 2) through a closure logic?
            # Or just rely on the fact that if controls are detached, update fails.
            import threading
            import time
            ...
            def run():
                while True:
                    time.sleep(3)
                    # Check if view is still active (hacky way: check if list is on page)
                    # But 'body.content' changes. 
                    # We need a robust way to stop polling.
                    # For now, let's just Refresh button or manual typing.
                    # User requested 'Chat', implying realtime. 
                    # Let's try to update if safe.
                    try:
                        if not load_messages(): break
                    except:
                        break
            
            # Threading is risky in Flet without page.run_task. 
            # We will skip auto-polling in this step to ensure stability first,
            # or use a Refresh Button.
            pass
         ...
         return ft.Column(
            ...
            controls=[
                ft.Container(
                    ...
                    content=ft.Row(
                        [
                            ft.Text("Comunidad (COS)", size=24, weight="bold", color=TEXT_COLOR, text_align="center", expand=True),
                            ft.Container(
                ...
                ft.Container(
                    ...
                    content=ft.Row([
                        input_field,
                        ft.Container(
                            content=ft.Icon("send", color=ACCENT_COLOR),
                            width=40, height=40,
                            alignment=ft.Alignment(0, 0),
                            border_radius=20,
    ...
    def view_profile():
        ...
        def handle_change_password(e):
             ...
             current_pass = current_pass_ref.current.value
             new_pass = new_pass_ref.current.value
             
             if not current_pass or not new_pass:
                 page.snack_bar = ft.SnackBar(ft.Text("Rellena ambos campos"))
                 page.snack_bar.open = True
                 page.update()
                 return
             ...
             if not username:
                 page.snack_bar = ft.SnackBar(ft.Text("Error: No has iniciado sesión"))
                 page.snack_bar.open = True
                 page.update()
                 return
                 
             success = api.change_password(username, current_pass, new_pass)
             
             if success:
                 page.snack_bar = ft.SnackBar(ft.Text("Contraseña actualizada con éxito"))
                 current_pass_ref.current.value = ""
                 new_pass_ref.current.value = ""
                 page.snack_bar.open = True
             else:
                 page.snack_bar = ft.SnackBar(ft.Text("Error: Contraseña actual incorrecta"))
                 page.snack_bar.open = True
             page.update()
        ...
        def handle_export(e):
             ...
             csv_data = api.get_export_csv()
             if csv_data:
                 # Save to Desktop automatically
                 try:
                     import os
                     # Get Desktop path reliably on Windows
                     desktop = os.path.join(os.environ['USERPROFILE'], 'Desktop')
                     folder = os.path.join(desktop, 'Antigravity')
                     filepath = os.path.join(folder, 'kk_logs.csv')
                     ...
                     page.snack_bar = ft.SnackBar(ft.Text(f"Logs guardados en: {filepath}"))
                 except Exception as ex:
                      page.snack_bar = ft.SnackBar(ft.Text(f"Error al guardar archivo: {ex}"))
             else:
                 page.snack_bar = ft.SnackBar(ft.Text("Error al descargar logs."))
             
             page.snack_bar.open = True
             page.update()

        current_pass_ref = ft.Ref[ft.TextField]()
        new_pass_ref = ft.Ref[ft.TextField]()
                ...
                ft.Container(
                    ...
                    content=ft.Column([
                        ...
                        ft.Container(
                            ...
                            visible=(current_user["username"] == "admin")
                        ),
                        
                        ft.Divider(color="grey"),
                        ft.Container(height=10),
                        ft.Text("Cambiar Contraseña", size=16, weight="bold", color=TEXT_COLOR),
                        ft.Container(height=10),
                        
                        # Change Password UI
                        ft.TextField(label="Contraseña Actual", password=True, can_reveal_password=True, bgcolor="#2C2C2E", color=TEXT_COLOR, border_radius=10, width=260, text_size=14, ref=current_pass_ref),
                        ft.TextField(label="Nueva Contraseña", password=True, can_reveal_password=True, bgcolor="#2C2C2E", color=TEXT_COLOR, border_radius=10, width=260, text_size=14, ref=new_pass_ref),
                        ft.Container(height=10),
                        ft.FilledButton("Actualizar Contraseña", on_click=handle_change_password, bgcolor=ACCENT_COLOR, color="black", width=260),
        ...
        return ft.Container(
            ...
            content=ft.Row(
                controls=[
                    ...
                    create_nav_item("groups", "COS", 2, current_index, image_src="assets/chivi.png"),
